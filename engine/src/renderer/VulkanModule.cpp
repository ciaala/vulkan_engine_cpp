...
void vlk::VulkanModule::prepareDescriptors(std::vector<vk::PipelineShaderStageCreateInfo> &shaderStageInfoList) {
    ...
    for (uint32_t i = 0; i < this->swapchainImageCount; ++i) {
        ...
        swapchain_image_resources[i].subCommands.resize(subCommandBufferAllocateInfo.commandBufferCount);
        result = device.allocateCommandBuffers(&subCommandBufferAllocateInfo,
...
void vlk::VulkanModule::draw(GameWorld *gameWorld) {
    ...
    for (auto &&gameObject : gameWorld->getGameObjects()) {
        //LOG(INFO) << "Updating gameObject: " << gameObject->getSid() << std::endl;
        if (gameObject->getSid() == 1) {
